...
These are deliberately out of scope. They are listed so the project does not drift into them, and revisited in Section 8.
...
- [x] **5.7 Commit baseline results** as `examples/pdf_extraction/BASELINE.md` with per-field accuracy, cost per document, and observed failure modes.
  - **Recorded 2026-09-22 from `run-8817ecd99ae6`,** one run of `jev_extraction_eval.py` — span selection with TypeSafe's `jev-1.13.0`, where a regex finds candidates and the model picks among them. 180/180 scalar fields, 30/30 invoices, $0.0015 total, recorded from a clean tree at `5634a85`. Line items are not scored by this method, and the document says so beside the table. The generative `extraction_eval.py` baseline still awaits an OpenAI key.
  - Producing it found a defect the mocked tests never could: `cost per document` printed `$0.0000` for a real $0.00005. Nine renderers each rounded money with their own `:.4f`, so any sub-hundredth-of-a-cent call — including an ordinary short gpt-4o-mini judge call — was shown as free. Fixed with one `format_usd`, and a test that fails on money formatted anywhere else.
...
Recorded here so the ideas are not lost, and so they are not built now. **Do not start any of these until Phase 7 is complete and released.**
