...
- **Reserved on PyPI as `0.0.0.dev0` on 2026-09-01**, since names in this space
  are being claimed quickly. Note that a `.dev` suffix does *not* stop
  `pip install evalstand` from installing the placeholder: pip skips
  pre-releases only when a stable release also exists. The placeholder is
  therefore yanked — a yanked release holds the name but is not installable
